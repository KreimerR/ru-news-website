...
export default function Economics() {
  const fullArticles = economicsArticlesData.map((article: TypeArticle) =>
    <Article
      key={article.id}
      id={article.id}
      image={article.image}
  ...
  for (let i = 1; i <= economicsArticlesData.length - 1; i++) {
  ...
  return (
    <div className="px-3 md:px-25 [@media(min-width:1700px)]:px-70">
      <h2 className="font-bold py-10">ЭКОНОМИКА</h2>

      <div className="lg:grid lg:grid-cols-[3fr_1fr] lg:gap-5">
        <div>
          <MainArticle
            id={economicsArticlesData[mArticle].id}
            image={economicsArticlesData[mArticle].image}
            title={economicsArticlesData[mArticle].title}
            category={economicsArticlesData[mArticle].category}
            author={economicsArticlesData[mArticle].author}
          />
