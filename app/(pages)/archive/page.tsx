...
import { articlesData, archiveArticlesData } from "@/data/articles"
import MainArticle from "@/components/MainArticle"
import Article from "@/components/Article"
import { popularArticlesParameters, mArticle } from "@/tools"

export default function Archive() {
  const fullArticles = archiveArticlesData.map((article: TypeArticle) =>
    <Article
      key={article.id}
      id={article.id}
      image={article.image}
  ...
  for (let i = 1; i <= archiveArticlesData.length - 1; i++) {
  ...
  return (
    <div className="px-3 md:px-25 [@media(min-width:1700px)]:px-70">
      <h2 className="font-bold py-10">АРХИВ</h2>

      <div className="lg:grid lg:grid-cols-[3fr_1fr] lg:gap-5">
        <div>
          <MainArticle
            id={archiveArticlesData[mArticle].id}
            image={archiveArticlesData[mArticle].image}
            title={archiveArticlesData[mArticle].title}
            category={archiveArticlesData[mArticle].category}
            author={archiveArticlesData[mArticle].author}
          />
