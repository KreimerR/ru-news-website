...
import { articlesData, opinionArticlesData } from "@/data/articles"
import MainArticle from "@/components/MainArticle"
import Article from "@/components/Article"
import { popularArticlesParameters, mArticle } from "@/tools"

export default function Opinions() {
  const fullArticles = opinionArticlesData.map((article: TypeArticle) =>
    <Article
      key={article.id}
      id={article.id}
      image={article.image}
  ...
  for (let i = 1; i <= opinionArticlesData.length - 1; i++) {
  ...
  return (
    <div className="px-3 md:px-25 [@media(min-width:1700px)]:px-70">
      <h2 className="font-bold py-10">МНЕНИЯ</h2>

      <div className="lg:grid lg:grid-cols-[3fr_1fr] lg:gap-5">
        <div>
          <MainArticle
            id={opinionArticlesData[mArticle].id}
            image={opinionArticlesData[mArticle].image}
            title={opinionArticlesData[mArticle].title}
            category={opinionArticlesData[mArticle].category}
            author={opinionArticlesData[mArticle].author}
          />
